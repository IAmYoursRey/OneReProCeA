    ...
    function animateIdle() {
        ...
        candles.forEach((c, index) => {
            ...
            if (!window.isBlownOut) {
                ...
                for (let p = 0; p < 15; p++) {
                    let puffElapsed = elapsed - (p * 80); 
                    
                    if (puffElapsed > 0 && puffElapsed < 1500) { 
                        let progress = puffElapsed / 1500; 

                        let smokeY = fy - (progress * 100) * scale; 

                        let smokeX = fx + Math.sin(progress * 6 + index) * 15 * scale; 

                        let smokeSize = (2 + progress * 12) * scale; 

                        let opacity = (1 - progress) * 0.4; 

                        window.ctx.beginPath();
                        window.ctx.arc(smokeX, smokeY, smokeSize, 0, Math.PI*2);
                        window.ctx.fillStyle = `rgba(200, 200, 200, ${opacity})`;
                        window.ctx.fill();
                    }
                }
            }
        ...
        if (!window.isBlownOut) {
            ...
            window.ctx.fillText("Tiup Lilin", window.width / 2, window.height / 2 + (250 * Math.min(1, window.width / 600)));
        }
